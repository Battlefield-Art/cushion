...
export function PaneTabBar({ paneId, tabs }: PaneTabBarProps) {
  ...
  const addNewTab = useWorkspaceStore((s) => s.addNewTab);
      ...
      >
        ...
        <DropdownMenuContent align="start" side="bottom" sideOffset={0}>
          <DropdownMenuItem onClick={() => {
            if (contextMenu) handleCloseTab(contextMenu.tabId);
          }}>
            Close
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => {
            if (contextMenu) handleCloseOthers(contextMenu.tabId);
          }}>
            Close others
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => {
            if (contextMenu) handleCloseToRight(contextMenu.tabId);
          }}>
            Close to the right
          </DropdownMenuItem>
          <DropdownMenuSeparator />
